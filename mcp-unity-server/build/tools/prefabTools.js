...
export function registerApplyPrefabOverridesTool(server, mcpUnity, logger) {
    logger.info(`Registering tool: ${applyPrefabToolName}`);
    server.tool(applyPrefabToolName, applyPrefabToolDescription, applyPrefabParamsSchema.shape, async (params) => {
        try {
...
async function applyPrefabToolHandler(mcpUnity, params) {
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to apply prefab overrides`);
    }
    return {
        content: [{
...
export function registerRevertPrefabOverridesTool(server, mcpUnity, logger) {
    logger.info(`Registering tool: ${revertPrefabToolName}`);
    server.tool(revertPrefabToolName, revertPrefabToolDescription, revertPrefabParamsSchema.shape, async (params) => {
        try {
...
async function revertPrefabToolHandler(mcpUnity, params) {
    validateGameObjectIdentifier(params);
    const response = await mcpUnity.sendRequest({
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to revert prefab overrides`);
    }
    return {
        content: [{
                type: response.type || 'text',
...
export function registerUnpackPrefabTool(server, mcpUnity, logger) {
    logger.info(`Registering tool: ${unpackPrefabToolName}`);
    server.tool(unpackPrefabToolName, unpackPrefabToolDescription, unpackPrefabParamsSchema.shape, async (params) => {
        try {
...
async function unpackPrefabToolHandler(mcpUnity, params) {
    validateGameObjectIdentifier(params);
    const response = await mcpUnity.sendRequest({
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to unpack prefab`);
    }
    return {
        content: [{
                type: response.type || 'text',
