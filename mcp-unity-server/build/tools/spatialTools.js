...
async function callTool(mcpUnity, method, params, failure) {
    const response = await mcpUnity.sendRequest({ method, params }, { timeout: getToolTimeout(method) });
    if (!response.success)
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || failure);
...
export function registerGetBoundsTool(server, mcpUnity, logger) {
...
export function registerPlaceNextToTool(server, mcpUnity, logger) {
...
export function registerFindLocalAssetsTool(server, mcpUnity, logger) {
...
export function registerImportLocalFileTool(server, mcpUnity, logger) {
...
export function registerMeasureDistanceTool(server, mcpUnity, logger) {
...
export function registerGetFloorHeightTool(server, mcpUnity, logger) {
...
export function registerGetNearbyObjectsTool(server, mcpUnity, logger) {
...
export function registerFrameCameraOnTool(server, mcpUnity, logger) {
...
// ============================================================================
export function registerSpatialTools(server, mcpUnity, logger) {
