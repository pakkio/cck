...
export function registerManageOcclusionCullingTool(server, mcpUnity, logger) {
    logger.info(`Registering tool: ${occlusionToolName}`);
    server.tool(occlusionToolName, occlusionToolDescription, occlusionParamsSchema.shape, async (params) => {
        try {
...
async function occlusionToolHandler(mcpUnity, params) {
    const response = await mcpUnity.sendRequest({
        method: occlusionToolName,
        params
    }, { timeout: getToolTimeout(occlusionToolName) });
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to execute manage_occlusion_culling`);
    }
    return {
        content: [{
...
export function registerConfigureLODGroupTool(server, mcpUnity, logger) {
    logger.info(`Registering tool: ${lodToolName}`);
    server.tool(lodToolName, lodToolDescription, lodParamsSchema.shape, async (params) => {
        try {
...
async function lodToolHandler(mcpUnity, params) {
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to configure LODGroup`);
    }
    return {
        content: [{
                type: response.type || 'text',
...
export function registerConfigureCameraCullingTool(server, mcpUnity, logger) {
    logger.info(`Registering tool: ${cameraCullingToolName}`);
    server.tool(cameraCullingToolName, cameraCullingToolDescription, cameraCullingParamsSchema.shape, async (params) => {
        try {
...
async function cameraCullingToolHandler(mcpUnity, params) {
    const response = await mcpUnity.sendRequest({
        method: cameraCullingToolName,
        params
    }, { timeout: getToolTimeout(cameraCullingToolName) });
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to configure camera culling`);
    }
    return {
        content: [{
                type: response.type || 'text',
