...
function createColorSchema(description) {
    return z.object({
...
function createVector3Schema(description) {
    return z.object({
...
export function registerManageLightingTool(server, mcpUnity, logger) {
    logger.info(`Registering tool: ${manageLightingToolName}`);
    server.tool(manageLightingToolName, manageLightingToolDescription, manageLightingParamsSchema.shape, async (params) => {
        try {
...
async function manageLightingToolHandler(mcpUnity, params) {
    const response = await mcpUnity.sendRequest({
        method: manageLightingToolName,
        params
    }, { timeout: getToolTimeout(manageLightingToolName) });
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to execute manage_lighting`);
    }
    return {
        content: [{
...
export function registerConfigureLightProbeGroupTool(server, mcpUnity, logger) {
    logger.info(`Registering tool: ${probeGroupToolName}`);
    server.tool(probeGroupToolName, probeGroupToolDescription, probeGroupParamsSchema.shape, async (params) => {
        try {
...
async function configureLightProbeGroupToolHandler(mcpUnity, params) {
    const response = await mcpUnity.sendRequest({
        method: probeGroupToolName,
        params
    }, { timeout: getToolTimeout(probeGroupToolName) });
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to configure light probe group`);
    }
    return {
        content: [{
                type: response.type || 'text',
...
export function registerCreateReflectionProbeTool(server, mcpUnity, logger) {
    logger.info(`Registering tool: ${reflectionProbeToolName}`);
    server.tool(reflectionProbeToolName, reflectionProbeToolDescription, reflectionProbeParamsSchema.shape, async (params) => {
        try {
...
async function createReflectionProbeToolHandler(mcpUnity, params) {
    const response = await mcpUnity.sendRequest({
        method: reflectionProbeToolName,
        params
    }, { timeout: getToolTimeout(reflectionProbeToolName) });
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to create reflection probe`);
    }
    return {
        content: [{
                type: response.type || 'text',
