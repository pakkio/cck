...
function createVector2Schema(description) {
    return z.object({
        x: z.number().describe('X component'),
        y: z.number().describe('Y component')
    }).describe(description);
...
export function registerManageTerrainTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to execute manage_terrain`);
    }
    return {
        content: [{
