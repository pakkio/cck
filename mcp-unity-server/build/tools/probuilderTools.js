...
export function registerProBuilderCreateShapeTool(server, mcpUnity, logger) {
    logger.info(`Registering tool: ${createShapeToolName}`);
    server.tool(createShapeToolName, createShapeToolDescription, createShapeParamsSchema.shape, async (params) => {
        try {
...
async function createShapeToolHandler(mcpUnity, params) {
    const response = await mcpUnity.sendRequest({
        method: createShapeToolName,
        params
    }, { timeout: getToolTimeout(createShapeToolName) });
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to create ProBuilder shape`);
    }
    return {
        content: [{
...
export function registerProBuilderMeshOpTool(server, mcpUnity, logger) {
    logger.info(`Registering tool: ${meshOpToolName}`);
    server.tool(meshOpToolName, meshOpToolDescription, meshOpParamsSchema.shape, async (params) => {
        try {
...
async function meshOpToolHandler(mcpUnity, params) {
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to execute ProBuilder mesh operation`);
    }
    return {
        content: [{
                type: response.type || 'text',
