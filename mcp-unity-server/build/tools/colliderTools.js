...
export function registerConfigureCollidersTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    if (params.action !== 'create_physics_material' && params.instanceId === undefined && !params.objectPath) {
        throw new McpUnityError(ErrorType.VALIDATION, "Either 'instanceId' or 'objectPath' is required for this collider action.");
    }
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to configure colliders`);
    }
    return {
        content: [{
