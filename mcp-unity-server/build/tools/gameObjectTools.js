...
}).refine(data => data.siblingIndex !== undefined || data.insertAfterInstanceId !== undefined || data.insertBeforeInstanceId !== undefined, { message: 'One of siblingIndex, insertAfterInstanceId, or insertBeforeInstanceId must be provided' });
/**
 * Creates and registers the Set Sibling Index tool with the MCP server
 */
export function registerSetSiblingIndexTool(server, mcpUnity, logger) {
    logger.info(`Registering tool: ${setSiblingIndexToolName}`);
    server.tool(setSiblingIndexToolName, setSiblingIndexToolDescription, 
...
async function setSiblingIndexHandler(mcpUnity, params) {
    if ((params.instanceId === undefined || params.instanceId === null) &&
        (!params.objectPath || params.objectPath.trim() === '')) {
        throw new McpUnityError(ErrorType.VALIDATION, "Either 'instanceId' or 'objectPath' must be provided");
    }
    if (params.siblingIndex === undefined && params.insertAfterInstanceId === undefined && params.insertBeforeInstanceId === undefined) {
        throw new McpUnityError(ErrorType.VALIDATION, "One of 'siblingIndex', 'insertAfterInstanceId', or 'insertBeforeInstanceId' must be provided");
    }
    const response = await mcpUnity.sendRequest({
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || 'Failed to set sibling index');
    }
    return {
        content: [{
                type: response.type || 'text',
