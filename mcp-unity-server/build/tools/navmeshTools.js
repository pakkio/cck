...
export function registerManageNavMeshTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    if (['add_agent', 'add_obstacle'].includes(params.action) && params.instanceId === undefined && !params.objectPath) {
        throw new McpUnityError(ErrorType.VALIDATION, `Either 'instanceId' or 'objectPath' is required when action is '${params.action}'.`);
    }
    if (params.action === 'calculate_path' && (!params.startPosition || !params.targetPosition)) {
        throw new McpUnityError(ErrorType.VALIDATION, "'startPosition' and 'targetPosition' are required when action is 'calculate_path'.");
    }
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to execute manage_navmesh`);
    }
    return {
        content: [{
