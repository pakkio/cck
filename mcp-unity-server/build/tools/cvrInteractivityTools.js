...
export function registerConfigureCvrInteractivityTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    if (params.instanceId === undefined && !params.objectPath) {
        throw new McpUnityError(ErrorType.VALIDATION, "Either 'instanceId' or 'objectPath' is required for configuring interactivity.");
    }
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to configure CVR interactivity`);
    }
    return {
        content: [{
