...
export function registerConfigurePostProcessingTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    if (params.action === 'add_override' && !params.effectType) {
        throw new McpUnityError(ErrorType.VALIDATION, "Parameter 'effectType' is required when action is 'add_override'.");
    }
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to configure post processing`);
    }
    return {
        content: [{
