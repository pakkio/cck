...
export function registerConfigureTextureSettingsTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    if (!params.texturePath) {
        throw new McpUnityError(ErrorType.VALIDATION, "Parameter 'texturePath' must be provided.");
    }
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to configure texture settings`);
    }
    return {
        content: [{
