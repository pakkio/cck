...
export function registerPhysicsAndComponentTools(server, mcpUnity, logger) {
    logger.info(`Registering tool: ${raycastToolName}`);
    server.tool(raycastToolName, raycastToolDescription, raycastParamsSchema.shape, async (params) => {
        try {
    ...
    server.tool(copyComponentToolName, copyComponentDescription, copyComponentParamsSchema.shape, async (params) => {
        try {
...
async function raycastQueryHandler(mcpUnity, params) {
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || 'Failed to execute raycast');
    }
...
async function copyComponentHandler(mcpUnity, params) {
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || 'Failed to copy component');
    }
    return {
        content: [{
                type: 'text',
