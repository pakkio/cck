...
export function registerAssetManagementTools(server, mcpUnity, logger) {
    ...
    server.tool(renameToolName, renameToolDescription, renameParamsSchema.shape, async (params) => {
        try {
    ...
    server.tool(createFolderToolName, createFolderDescription, createFolderParamsSchema.shape, async (params) => {
        try {
...
async function deleteAssetHandler(mcpUnity, params) {
    if (!params.assetPath) {
...
async function renameAssetHandler(mcpUnity, params) {
...
async function createFolderHandler(mcpUnity, params) {
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || 'Failed to create folder');
    }
    return {
        content: [{
                type: 'text',
