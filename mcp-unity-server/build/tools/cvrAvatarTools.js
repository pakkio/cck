...
export function registerManageCvrAvatarTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    if (params.instanceId === undefined && !params.objectPath) {
        throw new McpUnityError(ErrorType.VALIDATION, "Either 'instanceId' or 'objectPath' is required for managing CVR avatar.");
    }
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to manage CVR avatar`);
    }
    return {
        content: [{
