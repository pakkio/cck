...
export function registerExportPackageTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    if (!params.assetPath && (!params.assetPaths || params.assetPaths.length === 0)) {
        throw new McpUnityError(ErrorType.VALIDATION, "Either 'assetPath' or 'assetPaths' must be provided");
    }
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || 'Failed to export package');
    }
    return {
        content: [{
