...
export function registerBuildAndAnalysisTools(server, mcpUnity, logger) {
    logger.info(`Registering tool: ${buildToolName}`);
    server.tool(buildToolName, buildToolDescription, buildParamsSchema.shape, async (params) => {
        try {
    ...
    server.tool(getErrorsToolName, getErrorsToolDescription, getErrorsParamsSchema.shape, async (params) => {
        try {
    ...
    server.tool(findReferencesToolName, findReferencesToolDescription, findReferencesParamsSchema.shape, async (params) => {
        try {
...
async function buildProjectHandler(mcpUnity, params) {
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || 'Failed to build project');
    }
    return {
        content: [{
                type: 'text',
...
async function getCompilationErrorsHandler(mcpUnity, params) {
    const response = await mcpUnity.sendRequest({
        method: getErrorsToolName,
        params
    });
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || 'Failed to get compilation errors');
    }
    return {
        content: [{
                type: 'text',
...
async function findScriptReferencesHandler(mcpUnity, params) {
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || 'Failed to find script references');
    }
    return {
        content: [{
                type: 'text',
