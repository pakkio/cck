...
export function registerManageTagsAndLayersTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    if (['add_tag', 'add_layer', 'add_sorting_layer'].includes(params.action) && !params.name) {
        throw new McpUnityError(ErrorType.VALIDATION, `'name' is required when action is '${params.action}'.`);
    }
    if (params.action === 'set_object_layer' && params.instanceId === undefined && !params.objectPath) {
        throw new McpUnityError(ErrorType.VALIDATION, "Either 'instanceId' or 'objectPath' is required when action is 'set_object_layer'.");
    }
    if (params.action === 'set_collision_matrix' && !params.layerA && params.layerAIndex === undefined) {
        throw new McpUnityError(ErrorType.VALIDATION, "Either 'layerA' or 'layerAIndex' is required when action is 'set_collision_matrix'.");
    }
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to manage tags and layers`);
    }
    return {
        content: [{
